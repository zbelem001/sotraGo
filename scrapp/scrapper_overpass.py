...
def get_sotraco_data():
    ...
    [out:json][timeout:90];
    (
      relation["route"="bus"]["operator"~"SOTRACO", i];
      relation["route"="bus"]["network"~"SOTRACO", i];
    );
    out geom;
    """

    headers = {
        'User-Agent': 'SotracoApp/1.0 (Contact: test@example.com)'
    }
    ...
    for url in ENDPOINTS:
        ...
        try:
            response = requests.post(url, data={'data': overpass_query}, headers=headers, timeout=100)
            ...
            print(f"❌ Échec avec ce serveur ({str(e)[:50]})...")
            time.sleep(2)

    if not data:
        print("❌ Impossible de récupérer les données après avoir essayé tous les serveurs.")
        return
    ...
    for el in elements:
        if el['type'] == 'relation':
            ...
            for member in el.get('members', []):
                ...
                # C'est un arrêt ou une plateforme de bus
                ...
                # C'est un segment de la route (LineString)
                elif member['type'] == 'way':
                    geometry = member.get('geometry', [])
                    if geometry:
                        # latlong2 dans flutter prend longitude / latitude == [lon, lat]
